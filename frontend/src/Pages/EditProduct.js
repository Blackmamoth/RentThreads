...
const EditProduct = () => {
  ...
  const [newPrice, setNewPrice] = useState(product.rentPerDay);
  const clothId = product._id;
  ...
  const handleEdit = (e) => {
    ...
    axios
      .patch("http://localhost:5000/tl/clothes/update-clothes", item)
      .then((res) => {
  ...
  const handleDelete = (e) => {
    ...
    const proceed = window.confirm(
      `${title} will be deleted from your inventory`
    );
    if (proceed) {
      axios
        .delete("http://localhost:5000/tl/clothes/delete-cloth", {
          data: { clothId },
        })
        .then((res) => {
          showSuccessToast(res.data.data.message);
                  ...
                  <div className="shadow sm:overflow-hidden sm:rounded-md">
                    ...
                    <div className="bg-gray-50 px-4 py-3 text-right sm:px-6">
                      ...
                      <button
                        ...
                        className=" ml-3 inline-flex justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-red-700"
                      >
                        Delete
                      </button>
                    </div>
