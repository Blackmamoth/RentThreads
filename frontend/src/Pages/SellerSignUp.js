...
const SellerSignUp = () => {
 ...
 async function registerRequest() {
    // if(validateUserData()){
     ...
     await axios.post("http://localhost:5000/tl/auth/register", user).then(response => {
  ...
  async function validateUserData(e) {
    e.preventDefault()
    if (!user.username || !user.storeName || !user.email || !user.password || !user.passwordConfirmation    ) {
  ...
  const [user, setUser] = useState({ username: '', storeName: '', email: '', password: '', passwordConfirmation: '' })
              ...
              <form action="#" className="mt-8 grid grid-cols-6 gap-6">
                ...
                <div className="col-span-6 sm:flex sm:items-center sm:gap-4">
                  ...
                  <p className="mt-4 text-sm text-gray-500 sm:mt-0">
                    ...
                    <Link to="/seller/signin" className="text-gray-700 underline">Log in</Link>.
                  </p>
                </div>
